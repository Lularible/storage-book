...
`knotfs_get_result()`返回的是`cur.result`，即最后一个完成请求的返回值。0是成功，负值是错误码。
...
这个模式数据`pat[i] = i & 0xFF`很聪明，因为每个字节的值就是它的偏移量（模256）。这样读回后做`memcmp`，任何一个比特的错位（比如偏移1字节的错写）都会立即被发现。8192字节 = 2个整块 + 0字节。正好验证了块边界的正确处理。
