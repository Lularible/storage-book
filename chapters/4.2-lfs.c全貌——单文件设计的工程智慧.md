...
在LittleFS的设计哲学中，**每个模块的“公开接口”是在lfs.h中暴露给用户的那些`lfs_mount`、`lfs_file_open`、`lfs_dir_read`函数，它们构成了稳定API。** 内部模块之间通过`static`函数调用彼此，编译器做死代码消除（dead code elimination）后，未使用的内部函数不会出现在最终二进制中。
...
**顶层API**，即`lfs_mount`、`lfs_file_open`、`lfs_file_read`、`lfs_file_write`、`lfs_dir_open`、`lfs_dir_read`……这些公开接口都在文件末尾。它们基本上是对内部`_`后缀函数的薄封装：
...
**坏块重试循环。** 在写入代码中广泛使用了`relocate`标签，当检测到写入校验失败时，不返回错误，而是自动尝试分配一个新块重写：
...
你看到了lfs.c的全貌，单文件设计是嵌入式编译单元的理性选择。但LittleFS真正的黑魔法，藏在那些只有两块的metadata pair里。两块Flash怎么实现原子提交？为什么一定是两块而不是三块？revision计数器如何用“序列号算术”来避免整数溢出？下一节，我们钻进lfs_dir_commit和lfs_dir_compact的内部，看一个8字节的CRC是如何保证任何时刻目录都有合法状态的。

**悬念留给：当一个metadata pair写满时，它不报错，而是默默分叉成两个metadata pair，中间用一条尾巴连起来。这个过程如果在中间掉电，会发生什么？**
