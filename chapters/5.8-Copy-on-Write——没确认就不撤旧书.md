...
这里的关键：**compact写入的是非活跃槽位，如果中途断电，活跃槽位的旧SB不受任何影响。** 下次mount时，select_SB函数会挑选sequence号更大的那个SB——因为compact_write把新SB的sequence设为`sb.sequence + 1`但compact_finish还没来得及执行，所以Flash上的两个SB中，旧的那个sequence大（因为它已经被内存里的`sb.sequence`引用），新的那个sequence小——mount会选择旧的那个。旧SB里的file entry仍然指向旧数据。CoW的原子性，在这一刻得到了最终的保证。
