...
KnotFS是一个教学级文件系统。它的目标是跑在你的大脑里。它用约1770行纯C代码（核心1165行），不依赖任何外部库（除了libc），在一块用RAM数组模拟的64KB NOR Flash上，完整地实现了一个异步、日志结构化、掉电安全的文件系统。
...
KnotFS的全称是“Knot Filesystem”，即“绳结”文件系统。这个名字不是随便起的。
...
**KnotFS是本书的“教学伴侣”。** 你读到的每一个概念，包括超级块双副本、Log追加、Copy-on-Write、磨损均衡，在KnotFS里都有直截了当的实现。你不需要交叉编译工具链，不需要JTAG调试器，甚至不需要Linux内核源码。
...
knotfs.c          (1165行) —— 核心实现：状态机×8、Flash模拟器、CRC32
knotfs_test.c     (474行)  —— 12个自测用例，包括掉电恢复
Makefile          (71行)   —— 单文件编译，支持x86/ARM64/ARM32三架构
```

总共约1770行代码。如果你有C语言基础，一个下午就能通读一遍。但理解它为什么这样写，比如为什么状态机要这么设计，为什么超级块要存两份，为什么compact要在50%满时触发，这需要你读完本章剩下的几节。
...
注意那个`tick 15... tick 30...`。这就是异步文件系统的“心跳”，也就是每15个tick打印一次进度，让你“看到”Flash操作在慢慢推进。如果没有这个tick模拟，现代CPU会在0.1毫秒内跑完全部操作，你什么也看不到。
...
KnotFS让你在笔记本电脑上用`while(knotfs_run())`模拟这个场景。每一次函数返回，就相当于一次任务切换。你看着tick数字增长，渐渐理解：哦，原来异步跟多线程没有关系，它只是把一个长操作切成了很多个小步骤。
...
超级块（Superblock）是文件系统的“馆藏总目录”，记录了Flash上所有块的分配情况、文件目录、磨损计数。KnotFS存了两份：SB0在块0，SB1在块1。元数据的变更以8字节的Log记录追加到超级块的末尾，而非原地更新。这保证了“在最坏的情况下，也就是写入过程中断电，你总能找到一份完好的超级块”。
...
生产级方案会支持间接块（indirect block），每个文件可以有远超8个数据块。KnotFS去掉了间接块机制，每个文件最多8个直接块（`KNOTFS_DIRECT_BLKS = 8`），即32KB（`8 × 4096`）。这个限制对于教学来说足够了，因为大部分测试文件只有几十到几百字节。
...
因为KnotFS做了一件很罕见的事：它把嵌入式文件系统的核心思想，也就是异步状态机、日志结构化、Copy-on-Write、双副本冗余，从生产级设计理念中提炼出来，压缩到一千多行，同时保留了全部关键设计。

你读的不只是C代码。你读的是一个工程师在说：“你看，文件系统其实没那么神秘。你看这个超级块的结构，它就是一个C结构体。你看这个状态机，它就是一个switch语句。你看这个compact，它就是一次memcpy加一次erase。”

当你理解完这约1770行代码后，再看生产级方案的设计，你会惊讶地发现：**它们的内核是一样的。** 生产级方案多出来的代码，大部分是集成胶水（FreeRTOS任务管理、AUTOSAR驱动调用、缓存一致性维护），而不是文件系统逻辑本身。
