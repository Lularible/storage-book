...
> ⚠️ **教学简化提示**：这里KnotFS的请求队列有容量4但没有超时检测。生产级方案的请求队列有**超时检测机制**——每种操作有独立的超时时间，超时后请求被标记为失败，任务被通知。第6章详细展开。
...
写了这么多代码，讲了这么多原理，你怎么知道它真的能跑？你怎么知道CoW断电恢复真的有效？你怎么知道CRC校验真的能发现问题？KnotFS附带了一套12个测试用例的自测试程序——format、write、read、append、delete、power-loss recovery……每一步都有代码。**下一节，也是本章的最后一节，我们把所有状态机串起来跑一遍，看12个测试用例如何验证我们前面讲过的每一个设计决策。** `./knotfs_test`，~318 行测试代码，18 个测试点——**全过，是对你这一章所有思考的最好的回答。**
