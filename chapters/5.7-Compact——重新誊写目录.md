...
但要注意：在KnotFS中，compact的实际触发是**每次需要持久化文件条目的操作（write/append/delete）结束后手动触发的**，而非由“log_count超过阈值”自动触发。
...
这段代码暴露了KnotFS的关键简化：它在每次write/append/delete操作后都触发一个完整的compact流程。它触发compact是为了把文件条目的变更（nodes[]中的name、size、block_count、blocks[]）持久化到Flash，跟Log是否满没有关系。
...
这步更新的是内存中的`sb`（当前活跃的SB镜像），让它与已经写到Flash上的内容保持一致。如果不做这一步，内存中的sb会认为自己是旧版本，下次compact可能会重复写到同一个槽位。
...
compact 解决了“元数据怎么安全落地”，但文件系统最核心的操作是“写文件”。KnotFS 写文件是一套 Copy-on-Write 原子事务，而非简单的“打开→写入→关闭”：先在新书架上排好书，等全部排好、校验通过，才把旧书架上的书撤下来。
